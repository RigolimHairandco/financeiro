...
// ATUALIZADO: Campos de texto em MAIÚSCULAS
const TransactionForm = ({ onSave, transactionToEdit, setTransactionToEdit, activeDebts, userId, expenseCategories = [], incomeCategories = [] }) => {
    ...
    useEffect(() => { if (isEditing) { setDescription(transactionToEdit.description); setAmount(transactionToEdit.amount); setType(transactionToEdit.type); setDate(transactionToEdit.timestamp.toDate().toISOString().split('T')[0]); if (transactionToEdit.type === 'expense') setCategory(transactionToEdit.category); else setIncomeSource(transactionToEdit.incomeSource); setLinkedDebtId(transactionToEdit.linkedDebtId || ''); } }, [transactionToEdit, isEditing]);
    useEffect(() => { if (!isEditing) { setCategory(expenseCategories?.[0]?.name || ''); } }, [expenseCategories, isEditing]);
    useEffect(() => { if (!isEditing) { setIncomeSource(incomeCategories?.[0]?.name || ''); } }, [incomeCategories, isEditing]);

    const resetForm = () => { setDescription(''); setAmount(''); setDate(new Date().toISOString().split('T')[0]); setTransactionToEdit(null); setLinkedDebtId(''); setCategory(expenseCategories?.[0]?.name || ''); setIncomeSource(incomeCategories?.[0]?.name || ''); setType('expense'); };
...
const ExpensePieChart = ({ data }) => { const chartData = useMemo(() => { const categoryTotals = data.filter(t => t.type === 'expense').reduce((acc, t) => { const category = t.category || 'Sem Categoria'; acc[category] = (acc[category] || 0) + t.amount; return acc; }, {}); return Object.entries(categoryTotals).map(([name, value]) => ({ name, value })); }, [data]); if (chartData.length === 0) { return <div className="text-center text-gray-500 py-10 h-[300px] flex items-center justify-center">Sem dados de despesas para exibir no gráfico.</div>; } const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#AF19FF', '#FF1943', '#19D4FF', '#FFD419', '#8884d8', '#82ca9d', '#d88488']; return ( <ResponsiveContainer width="100%" height={300}><PieChart><Pie data={chartData} dataKey="value" nameKey="name" cx="40%" cy="50%" innerRadius={60} outerRadius={80} fill="#8884d8" paddingAngle={5} labelLine={false} label={({ name, percent }) => `${(percent * 100).toFixed(0)}%`}>{chartData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip formatter={(value) => value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} /><Legend layout="vertical" verticalAlign="middle" align="right" iconType="circle" /></PieChart></ResponsiveContainer> ); };
...
const DebtForm = ({ onSave }) => {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); if (!description || !totalAmount || parseFloat(totalAmount) <= 0) return; await onSave({ description, totalAmount: parseFloat(totalAmount), paidAmount: 0, status: 'active', createdAt: Timestamp.now() }); resetForm(); };
    return ( <div className="bg-white p-6 rounded-2xl shadow-md"><h3 className="text-xl font-bold text-gray-800 mb-4">Adicionar Nova Dívida</h3><form onSubmit={handleSubmit} className="space-y-4"><div><label htmlFor="debt-description" className="block text-sm font-medium text-gray-600 mb-1">Descrição da Dívida</label><input id="debt-description" type="text" value={description} onChange={e => setDescription(e.target.value.toUpperCase())} placeholder="Ex: FINANCIAMENTO DO CARRO" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg uppercase" required /></div><div><label htmlFor="debt-total" className="block text-sm font-medium text-gray-600 mb-1">Valor Total (R$)</label><input id="debt-total" type="number" step="0.01" value={totalAmount} onChange={e => setTotalAmount(e.target.value)} placeholder="25000,00" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg" required /></div><button type="submit" className="w-full py-3 px-4 bg-teal-600 text-white font-bold rounded-lg hover:bg-teal-700">Adicionar Dívida</button></form></div> );
};
