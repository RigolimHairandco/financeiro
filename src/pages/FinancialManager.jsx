...
const FinancialManager = ({ user, onLogout, setNotification }) => {
    ...
    const handleDeleteBudget = async (budgetId) => { if (!budgetId) return; try { await deleteDoc(doc(db, `users/${user.uid}/budgets`, budgetId)); setNotification({message: 'Orçamento apagado!', type: 'success'});} catch (e) { console.error("Erro ao apagar orçamento:", e); setNotification({message: "Ocorreu um erro ao apagar o orçamento.", type: 'error'}); } };
