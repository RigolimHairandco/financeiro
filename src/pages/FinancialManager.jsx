...
const FinancialManager = ({ user, onLogout, setAlertMessage }) => {
    ...
    const handleDelete = async () => {
        ...
        const collectionName = `${type}s`; // transactions, debts, goals
        const docRef = doc(db, `users/${user.uid}/${collectionName}`, id);

        try {
            if (type === 'transaction' && itemToDelete.data?.linkedDebtId) {
                const debtRef = doc(db, `users/${user.uid}/debts`, itemToDelete.data.linkedDebtId);
                const debtDoc = await getDoc(debtRef);
                if (debtDoc.exists()) {
                    const debtData = debtDoc.data();
                    const newPaidAmount = debtData.paidAmount - itemToDelete.data.amount;
                    await updateDoc(debtRef, { paidAmount: newPaidAmount < 0 ? 0 : newPaidAmount, status: 'active' });
                }
            } else if (type === 'debt') {
    ...
    const handleContributeToGoal = async (goal, amount) => {
        ...
        batch.set(newTransactionRef, { description: `CONTRIBUIÇÃO: ${goal.name}`.toUpperCase(), amount: amount, type: 'expense', category: 'Poupança', timestamp: Timestamp.now(), isRecurring: false });
